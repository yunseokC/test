...
from krs.utils.anthropic_client import AnthropicClient
from krs.utils.krs_client import KrsGPTClient
...
class KrsMain:
    ...
    async def health_check(self, change_model=False, device='cpu', enable_mcp=True):
        ...
        if os.path.exists(LLMSTATE_PICKLE_FILEPATH) and not change_model:
            ...
            if continue_previous_chat == 'y':
                if enable_mcp:
                    krsllmclient = AnthropicClient(device=device)
                else:
            ...
            await krsllmclient.connect_to_server("krs/utils/mcp_server.py")
            self.continue_chat = False
        ...
        await krsllmclient.interactive_session(prompt_to_llm)
